...
switch($_GET[act]){
  // Tampil Jaringan Bendung
  default:
    echo "<h4>Bendung</h4>";
	echo" <p><a href='?module=bendung&act=tambah&id=$_GET[id]';\" class='btn btn-success'>Tambah Bendung</a></p>";
    ...
    $posisi = $p->cariPosisi($batas);
      ...
      $tampil = mysql_query("SELECT * FROM bendungan WHERE jenis='B' ORDER BY id_bendung DESC LIMIT $posisi,$batas");
    ...
    while($r=mysql_fetch_array($tampil)){
      echo "<tr><td width='25'>$no</td>
				...
				<td>$r[das]</td>
				<td>$r[ws]</td>
				<td width='40'><a href=?module=bendung&act=edit&id=$r[id_bendung] title='Edit'><i class='icon-edit'></i></a>
		                <a href='$aksi?module=bendung&act=hapus&id=$r[id_bendung]&namafile=$r[booklet]' title='Hapus'><i class='icon-trash'></i> </a></td>
		        </tr>";
      $no++;
    }
    echo "</table>";

    $jmldata = mysql_num_rows(mysql_query("SELECT * FROM bendungan WHERE jenis = 'B'"));
...
echo "<div class=\"pagination\"><ul> $linkHalaman</ul></div>";
    ...
    $tampil = mysql_query("SELECT * FROM bendungan WHERE nama LIKE '%$_GET[kata]%' AND jenis = 'B' ORDER BY id_bendung DESC LIMIT $posisi,$batas");
    ...
    $jmldata = mysql_num_rows(mysql_query("SELECT * FROM bendungan WHERE nama LIKE '%$_GET[kata]%' AND jenis='B'"));
...
echo "<div class=\"pagination\"><ul> $linkHalaman</ul></div>";
  ...
  case "tambah":
    echo "<h4>Tambah Bendung</h4>
          <form method=POST action='$aksi?module=bendung&act=input' enctype='multipart/form-data'>
          <table class='table'><tbody>
          <tr><td>Kode Bendung</td>     <td><input type=text name='kode' class='input-small'></td></tr>
          <tr><td>Nama Bendung</td>     <td><input type=text name='nama' class='input-xxlarge'></td></tr>
          ...
          <tr><td>Tinggi Bendung (m)</td>  			<td><input type=text name='tinggi' class='small'></td></tr>
          <tr><td>Lebar Bendung (m)</td>  			<td><input type=text name='lebar' class='input-small'></td></tr>
  ...
  case "edit":
    ...
    echo "<h2>Edit Bendung</h2>
          <form method=POST enctype='multipart/form-data' action=$aksi?module=bendung&act=update>
          <input type=hidden name=id value=$r[id_bendung]>
          <table class='table'><tbody>
         <tr><td>Kode Bendung</td>     <td><input type=text name='kode' value='$r[kode_bendung]' class='input-small'></td></tr>
          <tr><td>Nama Bendung</td>     <td><input type=text name='nama' value='$r[nama]' class='input-xxlarge'></td></tr>
          ...
          <tr><td>Tinggi Bendung (m)</td>  			<td><input type=text name='tinggi' value='$r[tinggi_bendung]' class='small'></td></tr>
          <tr><td>Lebar Bendung (m)</td>  			<td><input type=text name='lebar' value='$r[lebar_bendung]' class='input-small'></td></tr>
