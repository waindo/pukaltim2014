...
$aksi="modul/mod_bendungan_irigasi/aksi_bendungan.php";
switch($_GET[act]){
  // Tampil Jaringan Bendungan
  default:
    echo "<h4>Bendungan</h4>";
	echo" <p><a href='?module=bendungan&act=tambah';\" class='btn btn-success'>Tambah Bendungan</a></p>";
    ...
    $posisi = $p->cariPosisi($batas);
      ...
      $tampil = mysql_query("SELECT * FROM bendungan WHERE jenis='BD' ORDER BY id_bendung DESC LIMIT $posisi,$batas");
  
    $no = $posisi+1;
    while($r=mysql_fetch_array($tampil)){
      echo "<tr><td width='25'>$no</td>
                <td>$r[kode_bendung]</td>
                <td>$r[nama]</td>
				<td>$r[sungai]</td>
				<td>$r[das]</td>
				<td>$r[ws]</td>
				<td width='40'><a href=?module=bendungan&act=edit&id=$r[id_bendung] title='Edit'><i class='icon-edit'></i></a>
		                <a href='$aksi?module=bendungan&act=hapus&id=$r[id_bendung]&namafile=$r[booklet]' title='Hapus'><i class='icon-trash'></i> </a></td>
		        </tr>";
      $no++;
    }
    echo "</table>";

    $jmldata = mysql_num_rows(mysql_query("SELECT * FROM bendungan WHERE jenis = 'BD'"));
...
echo "<div class=\"pagination\"><ul> $linkHalaman</ul></div>";
    ...
    $tampil = mysql_query("SELECT * FROM bendungan WHERE nama LIKE '%$_GET[kata]%' AND jenis = 'BD' ORDER BY id_bendung DESC LIMIT $posisi,$batas");
    ...
    $jmldata = mysql_num_rows(mysql_query("SELECT * FROM bendungan WHERE nama LIKE '%$_GET[kata]%' AND jenis='BD'"));
...
echo "<div class=\"pagination\"><ul> $linkHalaman</ul></div>";
  ...
  case "tambah":
    echo "<h4>Tambah Bendungan</h4>
          <form method=POST action='$aksi?module=bendungan&act=input' enctype='multipart/form-data'>
          <table class='table'><tbody>
          <tr><td>Kode Bendungan</td>     <td><input type=text name='kode' class='input-small'></td></tr>
          <tr><td>Nama Bendungan</td>     <td><input type=text name='nama' class='input-xxlarge'></td></tr>
          ...
          <tr><td>Tipe Bendung</td>  			<td><input type=text name='tipe' class='input-xxmedium'></td></tr>
          <tr><td>Tinggi Bendungan (m)</td>  			<td><input type=text name='tinggi' class='small'></td></tr>
          <tr><td>Panjang Bendungan (m)</td>  			<td><input type=text name='panjang' class='input-small'></td></tr>
  ...
  case "edit":
    $edit = mysql_query("SELECT * FROM bendungan WHERE id_bendung='$_GET[id]'");
    
    $r    = mysql_fetch_array($edit);

    echo "<h2>Edit Bendungan</h2>
          <form method=POST enctype='multipart/form-data' action=$aksi?module=bendungan&act=update>
          <input type=hidden name=id value=$r[id_bendung]>
          <table class='table'><tbody>
         <tr><td>Kode Bendungan</td>     <td><input type=text name='kode' value='$r[kode_bendung]' class='input-small'></td></tr>
          <tr><td>Nama Bendungan</td>     <td><input type=text name='nama' value='$r[nama]' class='input-xxlarge'></td></tr>
          ...
          <tr><td>Tipe Bendung</td>  			<td><input type=text name='tipe' value='$r[tipe_bendung]' class='input-xxmedium'></td></tr>
          <tr><td>Tinggi Bendungan (m)</td>  			<td><input type=text name='tinggi' value='$r[tinggi_bendung]' class='small'></td></tr>
          <tr><td>panjang Bendungan (m)</td>  			<td><input type=text name='panjang' value='$r[panjang_bendung]' class='input-small'></td></tr>
          <tr><td>Kapasitas Tampungan (m3)</td>  			<td><input type=text name='kapasitas' value='$r[kapasitas_tampung]' class='input-small'></td></tr>
