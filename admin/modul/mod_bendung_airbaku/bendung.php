...
$aksi="modul/mod_bendung_airbaku/aksi_bendung.php";
switch($_GET[act]){
  // Tampil Jaringan Bendung
  default:
    echo "<h4>Bendung</h4>";
	echo" <p><a href='?module=bendungairbaku&act=tambah';\" class='btn btn-success'>Tambah Bendung</a></p>";
    ...
    $posisi = $p->cariPosisi($batas);
      ...
      $tampil = mysql_query("SELECT * FROM air_baku WHERE jenis='Bendung' ORDER BY id_airbaku DESC LIMIT $posisi,$batas");
    ...
    while($r=mysql_fetch_array($tampil)){
      echo "<tr><td width='25'>$no</td>
				...
				<td>$r[das]</td>
				<td>$r[ws]</td>
				<td width='40'><a href=?module=bendungairbaku&act=edit&id=$r[id_airbaku] title='Edit'><i class='icon-edit'></i></a>
		                <a href='$aksi?module=bendungairbaku&act=hapus&id=$r[id_airbaku]&namafile=$r[booklet]' title='Hapus'><i class='icon-trash'></i> </a></td>
		        </tr>";
      $no++;
    }
    echo "</table>";

    $jmldata = mysql_num_rows(mysql_query("SELECT * FROM air_baku WHERE jenis = 'Bendung'"));
...
echo "<div class=\"pagination\"><ul> $linkHalaman</ul></div>";
    ...
    $tampil = mysql_query("SELECT * FROM air_baku WHERE nama LIKE '%$_GET[kata]%' AND jenis = 'Bendung' ORDER BY id_airbaku DESC LIMIT $posisi,$batas");
    ...
    $jmldata = mysql_num_rows(mysql_query("SELECT * FROM air_baku WHERE nama LIKE '%$_GET[kata]%' AND jenis='Bendung'"));
...
echo "<div class=\"pagination\"><ul> $linkHalaman</ul></div>";
  ...
  case "tambah":
    echo "<h4>Tambah Bendung</h4>
          <form method=POST action='$aksi?module=bendungairbaku&act=input' enctype='multipart/form-data'>
  ...
  case "edit":
    ...
    echo "<h2>Edit Bendung</h2>
          <form method=POST enctype='multipart/form-data' action=$aksi?module=bendungairbaku&act=update>
          <input type=hidden name=id value=$r[id_airbaku]>
          <table class='table'><tbody>
         <tr><td>Kode Bendung</td>     <td><input type=text name='kode' value='$r[kode_airbaku]' class='input-small'></td></tr>
          ...
          <tr><td>Tipe Bendung</td>  			<td><input type=text name='tipe' value='$r[tipe]' class='input-xxmedium'></td></tr>
          <tr><td>Tinggi Bendung (m)</td>  			<td><input type=text name='tinggi' value='$r[tinggi]' class='small'></td></tr>
          <tr><td>Lebar Bendung (m)</td>  			<td><input type=text name='lebar' value='$r[lebar]' class='input-small'></td></tr>
