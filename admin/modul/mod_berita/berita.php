...
switch($_GET[act]){
  ...
  default:
    echo "<h4>Berita</h4>";
	echo" <p><a href='?module=berita&act=tambahberita';\" class='btn btn-success'>Tambah Berita</a>
	<a href='?module=kategori';\" class='btn btn-success'>Kategori</a> <a href='?module=tag';\" class='btn btn-success'>Tag</a>
          ...
          Masukkan Judul Berita : <input type=text name='kata'> <input type=submit value=Cari class='btn'>
          </form></p>";
    if (empty($_GET['kata'])){
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td width='25'>$no</td>
                <td>$r[judul]</td>
                <td>$tgl_posting</td>
		            <td width='40'><a href=?module=berita&act=editberita&id=$r[id_berita] title='Edit'><i class='icon-edit'></i></a>
...
echo "<div class=\"pagination\"><ul> $linkHalaman</ul></div>";
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td>$no</td>
                <td>$r[judul]</td>
                <td>$tgl_posting</td>
		            <td><a href=?module=berita&act=editberita&id=$r[id_berita] class='btn btn-primary'><i class='icon-edit'></i> Edit</a>
		                <a href='$aksi?module=berita&act=hapus&id=$r[id_berita]&namafile=$r[gambar]' class='btn btn-danger'><i class='icon-trash'></i> Hapus</a></td>
		        </tr>";
      $no++;
    }
    echo "</table>";
...
echo "<div class=\"pagination\"><ul> $linkHalaman</ul></div>";
  ...
  case "tambahberita":
    ...
    echo "</select></td></tr>
          ...
          <tr><td>Isi Berita</td>  <td> <textarea name='isi_berita' class='ckeditor' style='width: 800px; height: 350px;'></textarea></td></tr>
  ...
  case "editberita":
   ...
   if ($r[headline]=='Y'){
      ...
      echo "<tr><td>Isi Berita</td>   <td> <textarea class='ckeditor' name='isi_berita' style='width: 800px; height: 350px;'>$r[isi_berita]</textarea></td></tr>
          <tr><td>Gambar</td>       <td> :  ";
          if ($r[gambar]!=''){
              echo "<img src='../assets/foto_berita/small_$r[gambar]'>";  
          }
    echo "</td></tr>
