...
switch($_GET[act]){
  // Manajemen air
  default:
    echo "<h2>Tabel Kebutuhan Air</h2>
          <input type=button value='input data' 
          onclick=\"window.location.href='?module=air&act=input';\">
          <table border = 1>
          <tr><th>No</th><th>Tahun</th><th>Bulan</th><th>Wilayah Sungai</th><th>Jenis Kebutuhan Air</th><th>Jumlah Kebutuhan Air</th><th>update</th></tr>"; 
	...
	echo "<h2>Tabel Ketersediaan Air</h2>
          <input type=button value='input data' 
          onclick=\"window.location.href='?module=air&act=inputdebit';\">
          <table border = 1>
          <tr><th>No</th><th>Tahun</th><th>Bulan</th><th>Wilayah Sungai</th><th>Jumlah Ketersediaan Air</th><th>update</th></tr>"; 
    $tampil=mysql_query("SELECT id, YEAR(waktu) t ,MONTHNAME(waktu) m,ws,jumlah FROM debit_air ORDER BY waktu ASC");
    $no=1;
    while ($r=mysql_fetch_assoc($tampil)){
       echo "<tr><td>$no</td>
             <td>$r[t]</td>
			 ...
			 <td>$r[ws]</td>
			 <td>$r[jumlah]</td>
             <td><a href='?module=air&act=editdebit&id=$r[id]'>update</a> | <a href='$aksi?module=air&act=hapusdebit&id=$r[id]'>Hapus</a>
             </td></tr>";
      $no++;
    }
    echo "</table>";
  ...
  case "inputdebit":
    echo "<h2>Input Data Ketersediaan Air</h2>
          <form method=POST action='$aksi?module=air&act=inputdebit'>
          <table >
		  <tr><td>Waktu : <input id = 'Date' name='waktu'></td></tr>
          <tr><td>WS : <select name='WS'>
				<option value = 'WS Berau Kelai' > WS Berau Kelai </option>
		  ...
		  <tr><td>Jumlah Ketersediaan Air : <input type=text name='jumlah'></td></tr>
          <tr><td colspan=2><input type=submit name=submit value=Simpan>
                            <input type=button value=Batal onclick=self.history.back()></td></tr>
          </table></form>";
     break;
 ...
 // Form Edit air 
  case "editdebit":
    $edit=mysql_query("SELECT * FROM debit_air WHERE id='$_GET[id]'");
    $r=mysql_fetch_array($edit);

    echo "<h2>Update Data Ketersediaan</h2>
          <form method=POST action=$aksi?module=air&act=updatedebit>
          <input type=hidden name=id value='$r[id]'>
          <table>
		  <tr><td>Jumlah</td><td> : <input type=text name='jumlah' value='$r[jumlah]'></td></tr>";
    echo "<tr><td colspan=2><input type=submit value=Update>
