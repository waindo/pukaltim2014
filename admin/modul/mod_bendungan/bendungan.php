...
$aksi="modul/mod_bendungan/aksi_bendungan.php";
switch($_GET[act]){
  // Tampil Jaringan Bendungan
  default:
    echo "<h4>Bendungan</h4>";
	echo" <p><a href='?module=bendunganairbaku&act=tambah';\" class='btn btn-success'>Tambah Bendungan</a></p>";
    ...
    $posisi = $p->cariPosisi($batas);
      ...
      $tampil = mysql_query("SELECT * FROM air_baku WHERE jenis='Bendungan' ORDER BY id_airbaku DESC LIMIT $posisi,$batas");
    ...
    while($r=mysql_fetch_array($tampil)){
      echo "<tr><td width='25'>$no</td>
				...
				<td>$r[das]</td>
				<td>$r[ws]</td>
				<td width='40'><a href=?module=bendunganairbaku&act=edit&id=$r[id_airbaku] title='Edit'><i class='icon-edit'></i></a>
		                <a href='$aksi?module=bendunganairbaku&act=hapus&id=$r[id_airbaku]&namafile=$r[booklet]' title='Hapus'><i class='icon-trash'></i> </a></td>
		        </tr>";
      $no++;
    }
    echo "</table>";

    $jmldata = mysql_num_rows(mysql_query("SELECT * FROM air_baku WHERE jenis = 'Bendungan'"));
...
echo "<div class=\"pagination\"><ul> $linkHalaman</ul></div>";
    ...
    $tampil = mysql_query("SELECT * FROM air_baku WHERE nama LIKE '%$_GET[kata]%' AND jenis = 'Bendungan' ORDER BY id_airbaku DESC LIMIT $posisi,$batas");
  
  
    $no = $posisi+1;
    while($r=mysql_fetch_array($tampil)){
        echo "<tr><td width='25'>$no</td>
                <td>$r[kode_airbakuac]</td>
                <td>$r[nama]</td>
				<td>$r[sungai]</td>
				<td>$r[das]</td>
				<td>$r[ws]</td>
    ...
    $jmldata = mysql_num_rows(mysql_query("SELECT * FROM air_baku WHERE nama LIKE '%$_GET[kata]%' AND jenis='Bendungan'"));
...
echo "<div class=\"pagination\"><ul> $linkHalaman</ul></div>";
  ...
  case "tambah":
    echo "<h4>Tambah Bendungan</h4>
          <form method=POST action='$aksi?module=bendunganairbaku&act=input' enctype='multipart/form-data'>
  ...
  case "edit":
    ...
    echo "<h2>Edit Bendungan</h2>
          <form method=POST enctype='multipart/form-data' action=$aksi?module=bendunganairbaku&act=update>
         ...
         <tr><td>Kode Bendungan</td>     <td><input type=text name='kode' value='$r[kode_airbaku]' class='input-small'></td></tr>
          ...
          <tr><td>Tinggi Bendungan (m)</td>  			<td><input type=text name='tinggi' value='$r[tinggi]' class='small'></td></tr>
          <tr><td>panjang Bendungan (m)</td>  			<td><input type=text name='panjang' value='$r[panjang]' class='input-small'></td></tr>
